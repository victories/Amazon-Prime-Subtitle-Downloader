...
// @description Allows you to download subtitles from Amazon Prime Video
// @license     MIT
// @version     3.0.0
// @namespace   github.com/victories
...
// @homepageURL https://github.com/victories/Amazon-Prime-Subtitle-Downloader
// @supportURL  https://github.com/victories/Amazon-Prime-Subtitle-Downloader/issues
// @downloadURL https://raw.githubusercontent.com/victories/Amazon-Prime-Subtitle-Downloader/main/Amazon%20Video%20-%20subtitle%20downloader.user.js
// @updateURL   https://raw.githubusercontent.com/victories/Amazon-Prime-Subtitle-Downloader/main/Amazon%20Video%20-%20subtitle%20downloader.user.js
// ==/UserScript==
      ...
      if(Array.isArray(actionData)) {
        for(const action of actionData) {
          if(action.payload && action.payload.playback && action.payload.playback.playbackEnvelope) {
            info.envelope = action.payload.playback.playbackEnvelope;
            info.expiry = action.payload.playback.expiryTime;
            envelopeFound = true;
            break;
          }
        }
      }
      // Old structure: playback.main.children[].playbackEnvelope
      else if(actionData && actionData.main && actionData.main.children) {
        for(const child of actionData.main.children) {
...
const extractProps = (data) => {
  ...
  // Old structure: data.props.body[0].props
  if(data && data.props && data.props.body && data.props.body[0]) {
    return data.props.body[0].props;
  }
  return undefined;
};

const extractActionsFromEntry = (action) => {
  // New structure: action has primaryActions array
  if(action.primaryActions) {
    return action.primaryActions;
  }
  // Old structure: action has playbackActions
  if(action.playbackActions) {
    return action.playbackActions;
  }
  return null;
};

const init = (url, fromNavigation) => {
  let props = undefined;

  if(typeof fromNavigation === "undefined") {
    if(INFO_URL !== null)
      return;

    INFO_URL = url;

    // Try new structure: script[type="application/json"]
    for(const scriptElement of document.querySelectorAll('script[type="application/json"]')) {
      let data;
      try {
        data = JSON.parse(scriptElement.innerHTML);
    ...
    // Fallback: old structure script[type="text/template"]
    if(typeof props === "undefined") {
    ...
    // SPA navigation data
    try {
      // New structure: direct body with atf/btf
      if(fromNavigation.init && fromNavigation.init.preparations) {
        props = extractProps(fromNavigation);
      }
      // Old structure: page[0].assembly.body[0].props
      else if(fromNavigation.page) {
        props = fromNavigation.page[0].assembly.body[0].props;
      }
    }
    catch(ignore) {}
  ...
  for(const [id, action] of Object.entries(props.atf.state.action.atf)) {
    const extracted = extractActionsFromEntry(action);
    if(extracted !== null) {
      actions.push([id, extracted]);
    }
  }
...
const parseEpisodes = data => {
  // New enrichItemMetadata API response structure
  if(data.widgets && data.widgets.episodeList && data.widgets.episodeList.episodes) {
    ...
    for(const episode of data.widgets.episodeList.episodes) {
      ...
      const extracted = extractActionsFromEntry(episode.action || {});
      if(extracted !== null) {
        actions.push([episode.titleID, extracted]);
      }
    }
    const parsedActions = parseActions(actions);
    addDownloadButtons(parsedActions);
  ...
  // New structure: enrichItemMetadata returns entities with detail/action data
  if(data.entities) {
    const actions = [];
    for(const entity of data.entities) {
      if(entity.titleID && entity.detail) {
        const pageTitleId = INFO_CACHE.keys().next().value;
        parseDetails(pageTitleId, {}, entity.titleID, entity.detail);
      }
      if(entity.titleID && entity.action) {
        const extracted = extractActionsFromEntry(entity.action);
        if(extracted !== null) {
          actions.push([entity.titleID, extracted]);
        }
      }
    }
    const parsedActions = parseActions(actions);
...
const injection = () => {
  ...
  ((open, realFetch) => {
    ...
    window.fetch = async (...args) => {
      ...
      else if(args[0] && args[0].includes("/api/enrichItemMetadata?")) {
        const copied = (await response).clone();
        const data = await copied.json();
        window.dispatchEvent(new CustomEvent("amazon_sub_downloader_data", {detail: {type: "episodes", data: data}}));
      }
